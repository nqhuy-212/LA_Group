...
export function CompanyTab({
  ...
  const [editing, setEditing] = useState<CompanyAdminOutDTO | null>(null);
  ...
  async function handleCreate(event: FormEvent<HTMLFormElement>) {
    ...
    const res = await browserFetch("/api/admin/companies", {
      method: "POST",
  ...
  async function handleUpdate(event: FormEvent<HTMLFormElement>) {
    ...
    setPending(true);
    const form = new FormData(event.currentTarget);
    const displayNamePublic = String(form.get("display_name_public") ?? "").trim();
    const logoUrl = String(form.get("logo_url") ?? "").trim();
    const res = await browserFetch(`/api/admin/companies/${editing.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        name: String(form.get("name") ?? "").trim(),
        display_name_public: displayNamePublic || null,
        logo_initials: String(form.get("logo_initials") ?? "").trim() || null,
  ...
  async function handleDelete(item: CompanyAdminOutDTO) {
    if (!confirm(`Xoá công ty "${item.name}"? Không thể hoàn tác.`)) return;
    const res = await browserFetch(`/api/admin/companies/${item.id}`, { method: "DELETE" });
    if (!res.ok) {
    ...
    <div>
          ...
          {items.map((item) => (
            ...
            >
              <div className="min-w-0">
                ...
                {item.display_name_public ? (
                  <span className="ml-2 text-xs text-text-muted">
                    (hiển thị công khai: {item.display_name_public})
                  </span>
                ) : null}
      ...
      >
        <form onSubmit={handleCreate} className="flex flex-col gap-4 p-5">
          <h2 className="text-base font-extrabold text-text">Thêm công ty</h2>
      ...
      >
        {editing ? (
          <form onSubmit={handleUpdate} className="flex flex-col gap-4 p-5">
            <h2 className="text-base font-extrabold text-text">Sửa công ty</h2>
            <label className={labelClass}>
              Tên công ty
              <input name="name" required defaultValue={editing.name} className={fieldClass} />
            </label>
            <label className={labelClass}>
              Tên hiển thị công khai (để trống nếu dùng luôn tên trên)
              <input
                name="display_name_public"
                defaultValue={editing.display_name_public ?? ""}
                className={fieldClass}
              />
            </label>
            <label className={labelClass}>
              Chữ viết tắt logo (VD: PT)
              <input
                name="logo_initials"
                maxLength={10}
                defaultValue={editing.logo_initials ?? ""}
                className={fieldClass}
              />
            </label>
            <label className={labelClass}>
