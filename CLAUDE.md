...
| `security.md` | **Auth backend đầy đủ xong** (P2): login/refresh/logout/me, cookie httpOnly+Secure+SameSite=Strict, khoá tài khoản sau 5 lần sai, refresh token xoay vòng + phát hiện tái sử dụng, `/docs` tắt ở prod. Chưa có UI đăng nhập (đó là P5); RBAC middleware Next.js cũng chờ P5. |
...
1. **PHASE 3 — API đọc công khai + FE bỏ mock data**: `app/schemas/{job,post,common}.py` + `app/api/v1/public/{jobs,posts,taxonomies}.py`, sau đó `openapi-typescript` codegen + lớp mapper phía frontend để thay `lib/mock-data.ts` bằng dữ liệu thật (D14).
2. Các phase sau theo đúng đồ thị phụ thuộc trong `docs/PLAN.md` (P4 SEO → P5 Admin đăng tin/go-live → P6 Form ứng tuyển → P7 Dashboard → P8 Chatbot → P9 Deploy).
