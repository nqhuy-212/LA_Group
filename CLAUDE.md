...
|---|---|
| `feature-recruitment.md` | Trang chủ render 100% từ API thật (P3) — danh sách/tabs lọc theo ngành vẫn client-side nhưng dữ liệu thật. Trang chi tiết việc làm riêng, tìm kiếm/lọc qua URL, form ứng tuyển — **chưa làm** (P4/P6). |
| `feature-chatbot-ai.md` | UI widget xong (demo, câu trả lời cứng). Tích hợp Claude API + RAG ở FastAPI — **chưa làm**. |
| `feature-company-content.md` | Khối tin tức/chính sách + feed doanh nghiệp mới/cảnh báo lừa đảo trên trang chủ nay lấy từ API thật (P3). Trang chi tiết bài viết + CMS thật — **chưa làm**. |
| `feature-admin-dashboard.md` | **Chưa bắt đầu** (route `(internal)`, JWT/RBAC, Dashboard tổng quan...). |
| `data-models.md` | 11 model SQLAlchemy + migration 0001 xong (P1). **API đọc công khai xong** (P3): `/api/jobs`, `/api/job-categories`, `/api/industrial-parks`, `/api/posts`, `/api/companies/recent`. |
| `security.md` | Auth backend đầy đủ xong (P2). Chưa có UI đăng nhập (đó là P5); RBAC middleware Next.js cũng chờ P5. |
| `tech-stack.md` / `commands.md` | Dev env xong (Python 3.11, Docker Postgres, scaffold FE/BE, dependency pin bằng uv, CI). `lib/mock-data.ts` **đã xoá** — frontend giờ 100% chạy trên API thật (P3). Stack production VPS (Nginx/Certbot/n8n) — **chưa làm**. |

### Bước tiếp theo

**Theo đúng thứ tự phụ thuộc ở [`docs/PLAN.md`](docs/PLAN.md) — nguồn sự thật duy nhất cho roadmap, đọc trước khi bắt đầu:**

1. **PHASE 4 — Trang chi tiết + danh sách + SEO** — đọc kỹ `frontend/AGENTS.md` trước (Next.js 16 có breaking change so với kiến thức huấn luyện): `app/(public)/viec-lam/page.tsx` + `viec-lam/[slug]/page.tsx`, `tin-tuc/page.tsx` + `[slug]/page.tsx`, `chinh-sach-bao-mat/page.tsx` (bắt buộc trước P6), JSON-LD `JobPosting`/`Organization`, `sitemap.ts`/`robots.ts`.
2. Các phase sau theo đúng đồ thị phụ thuộc trong `docs/PLAN.md` (P5 Admin đăng tin/go-live → P6 Form ứng tuyển → P7 Dashboard → P8 Chatbot → P9 Deploy).
3. Ghi chú riêng: xác nhận lại nghi vấn tràn lề ở `HeroSection` mobile 390px (xem mục P3 phía trên) trong một phiên có tập trung vào thiết kế.
